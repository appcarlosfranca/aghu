-- AGHU Notes v18 — correção dos contadores
-- Execute UMA VEZ no SQL Editor do Supabase.
-- Seguro para executar novamente. Não apaga notas.
...
-- A v18 permite SOMENTE leitura pública dos dois números.
-- Isso torna o contador no frontend mais confiável sem permitir alterações.
grant usage on schema public to anon,authenticated;
grant select on table public.app_counters to anon,authenticated;
...
for each row execute function public.count_note_created();

-- Conferência:
select
  access_count,
  notes_created_count,
  (select count(*) from public.note_creation_log) as notas_no_log
